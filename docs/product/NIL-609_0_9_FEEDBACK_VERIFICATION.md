...
| 3   | **trifft weiterhin zu**  | Im selben statischen Zustand ist die Hauptleiste weiss (`rgb(255, 255, 255)`), die sichtbare Flaeche des aktiven Auswahlwerkzeugs blau (`rgb(224, 223, 255)`) und die eines nicht ausgewaehlten Werkzeugs transparent. Hamburger, Timer und obere rechte Leiste sind grau (`rgb(236, 236, 244)`). Gemessen ist bei den Werkzeugen jeweils die farbtragende `.ToolIcon__icon`-Flaeche; das `data-testid` selbst sitzt auf einem transparenten Radio-Input. Die unterschiedliche Flaechenbehandlung ist damit weiterhin sichtbar. [Screenshot](../evidence/nil-609/03-button-colors-state-hierarchy.png)                                                                                                  |
...
- `frontend/src/pages/editor/EditorView.tsx` uebergibt `EditorTopRight` ueber Excalidraws
  `renderTopRightUI`-Slot. `frontend/src/pages/editor/EditorTopRight.tsx` rendert nur die
  ExcaliDash-Eintraege und beschreibt die von Excalidraw erzeugte DOM-Reihenfolge als
  `[avatars][this component's own children][Library trigger]`: Der benachbarte Library-Trigger
  wird also automatisch von Excalidraw gerendert, nicht von `ExcalidrawHost.tsx` oder einem
  exportierten `DefaultSidebarTrigger` in ExcaliDash.
