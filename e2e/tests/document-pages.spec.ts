 ...
 *
 * A maximum measures the noisiest scheduler interruption on the shared
 * runner, not the application's typical responsiveness. NIL-697 observed
 * 241 ms and 386 ms one-off maxima with ordinary 10 ms medians and sub-20 ms
 * p95 values on unrelated PRs. The p95 therefore remains the per-trial
 * product budget, and repeated sampling keeps one host outlier from deciding
 * the test. A genuine regression blocks the main thread often enough to lift
 * p95 on every attempt, so two of three still catches it. See
