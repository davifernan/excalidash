...
function createLiveAdapter(config, commands = {}) {
  const multica = commands.multica || ((...args) => commandJson("multica", [...args, "--output", "json"], { cwd: config.repoPath }));
  const gh = commands.gh || ((...args) => commandJson("gh", args, { cwd: config.repoPath }));
  const text = commands.text || commandText;
  ...
  return {
    ...
    getMainSha() {
      return text("gh", ["api", `repos/${config.repository}/git/ref/heads/main`, "--jq", ".object.sha"], { cwd: config.repoPath });
    },
    ...
    getPrImpactManifest(pr) {
      ...
      const files = pages.flatMap((page) => page.map((file) => file.filename));
...
async function scan({ adapter, config, now = new Date(), dryRun = false, mutationsPaused = false }) {
  ...
  if (!anyActiveImplementation && eligible.length > 0) {
    ...
    saveState(config.stateFile, state);
    const alreadyWokeMaster = Number(state.incidents[idleKey]?.recovery_attempts || 0) > 0;
    if (observation.shouldAct && !alreadyWokeMaster) {
      if (!suppressMutations) {
  ...
  if (duePackages.length > 0) {
    ...
    saveState(config.stateFile, state);
    const alreadyRequestedQa = Number(state.incidents[qaKey]?.recovery_attempts || 0) > 0;
    if (observation.shouldAct && !alreadyRequestedQa) {
