...
To run something that isn't on `main` yet, build from source instead (see "Quick setup: local
development" below), or check out that commit and run `docker compose up -d --build`.
