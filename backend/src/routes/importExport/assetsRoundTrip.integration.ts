...
import { createDatabaseBackup } from "../../backups/scheduler";
import {
  createTestUser,
  getTestPrisma,
  setupTestDb,
...
describe("document backup and export round trip", () => {
  ...
  // Runs under BOTH providers. It was SQLite-only until 02.09.2026, because the
  // scheduler genuinely could not back up PostgreSQL -- and that skip is why the
  // gap survived the release that made PostgreSQL the default: a skipped test
  // reports the same green as a passing one. The support now exists, so the
  // skip is gone and this asserts the provider's own snapshot name.
  it("scheduled backup stores the database and originals but excludes the render cache", async () => {
    ...
    // The snapshot's name follows the provider: a `pg_dump` archive is not a
    // SQLite file and must not be mistaken for one.
    expect(
      archive.file(testDatabaseProvider === "postgresql" ? "database.dump" : "database.sqlite"),
    ).toBeTruthy();
