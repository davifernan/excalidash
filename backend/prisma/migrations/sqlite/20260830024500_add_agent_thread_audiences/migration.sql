CREATE TABLE "AgentThread" (
    ...
    CONSTRAINT "AgentThread_drawingId_fkey" FOREIGN KEY ("drawingId") REFERENCES "Drawing" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT "AgentThread_contextId_fkey" FOREIGN KEY ("contextId") REFERENCES "AgentContext" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
...
-- A Context already owns a history. Reusing its id as the thread id makes the
-- data move deterministic on both providers without inventing SQL-side UUIDs.
...
CREATE TABLE "AgentThreadEvent" (
    ...
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT "AgentThreadEvent_threadId_fkey" FOREIGN KEY ("threadId") REFERENCES "AgentThread" ("id") ON DELETE CASCADE ON UPDATE CASCADE
);
