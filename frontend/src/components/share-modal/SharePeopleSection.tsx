...
type Props = {
  ...
  sharing: { permissions: api.DrawingPermissionRow[]; roster: api.DrawingRosterRow[] } | null;
  userQuery: string;
...
export const SharePeopleSection: React.FC<Props> = ({
  ...
  // NIL-291: people with a standing claim through the collection this board
  // lives in, not a direct grant on the board itself -- previously invisible
  // here entirely (`GET .../sharing` returned direct permissions only). The
  // access itself is unchanged by showing it (getDrawingAccess has resolved
  // both claims correctly since NIL-487); this is a transparency fix, not a
  // rights change, so the list below is read-only.
  const directGranteeIds = new Set((sharing?.permissions || []).map((p) => p.granteeUserId));
  const inherited = (sharing?.roster || []).filter(
    (member) =>
      member.via === "collection" &&
      member.userId !== currentUserId &&
      !directGranteeIds.has(member.userId),
  );
            ...
            {inherited.map((member) => (
              <div key={member.userId} className="flex items-center gap-3 px-1 py-1.5">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  ...
                  <div className="text-[10px] font-bold text-slate-400 dark:text-neutral-500 mt-0.5">
                    via the collection this board is in
                  </div>
                </div>
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-neutral-500 pr-1 shrink-0">
