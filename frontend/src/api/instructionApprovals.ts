...
export const getInstructionContexts = async (drawingId: string): Promise<InstructionContext[]> =>
  (await api.get<{ contexts: InstructionContext[] }>(`/drawings/${drawingId}/instruction-contexts`))
    .data.contexts;
...
export const getInstructionApproval = async (
  ...
  (
    await api.get(
      `/drawings/${drawingId}/instruction-contexts/${contextId}/instructions/${elementId}/approval`,
    )
  ).data;
...
export const previewInstructionApproval = async (
  ...
  (
    await api.get(
      `/drawings/${drawingId}/instruction-contexts/${contextId}/instructions/${elementId}/approval-preview`,
    )
  ).data;
